# Master Download Script for Kalamandir Images
...
foreach ($cat in $categories.Keys) {
    if (!(Test-Path $cat)) { New-Item -ItemType Directory -Path $cat }
    $image_urls = $categories[$cat]
    for ($i = 0; $i -lt $image_urls.Count; $i++) {
        $prefix = switch ($cat) {
            "mensformal" { "formal" }
            "menscasual" { "casual" }
            "mensethnic" { "ethnic" }
            ...
            "lingirie" { "bra" } # Based on existing filenames
            "sarees" { "saree" }
            "kids" { "kids" }
        ...
        $destFile = Join-Path $cat "$($prefix)$($i+1).jpg"
        try {
            Write-Host "Downloading $($image_urls[$i]) to $destFile ..."
            $wc.DownloadFile($image_urls[$i], $destFile)
            Write-Host "Success."
        } catch {
            Write-Host "Failed to download $($image_urls[$i]): $($_.Exception.Message)"
        }
    }
}
